...
export const LessonContentView: React.FC<LessonContentViewProps> = ({
  ...
  const videoResources = React.useMemo(() => lesson.resources.filter((r) => r.resourceType === 0), [lesson.resources]);
  const audioResources = React.useMemo(() => lesson.resources.filter((r) => r.resourceType === 1), [lesson.resources]);
  const pdfResources = React.useMemo(() => lesson.resources.filter((r) => [2, 3, 4, 5].includes(r.resourceType)), [lesson.resources]);
  
  // Top bar only displays videos; if no videos, fallback to displaying documents
  const topBarResources = React.useMemo(() => {
    return videoResources.length > 0 ? videoResources : pdfResources;
  }, [videoResources, pdfResources]);
  ...
  // Set default preview document when switching lessons or pdfResources
  useEffect(() => {
    if (pdfResources.length > 0) {
      if (!previewDocId || !pdfResources.some((d) => d.id === previewDocId)) {
        setPreviewDocId(pdfResources[0].id);
      }
    } else {
      setPreviewDocId(null);
    }
  }, [lesson.id, pdfResources, previewDocId]);

  const selectedPreviewDoc = pdfResources.find((d) => d.id === previewDocId) || pdfResources[0] || null;
  ...
  const currentPrimaryResource = React.useMemo(() => {
    ...
    const video = videoResources[0];
    if (video) return video;
    const doc = pdfResources[0];
    if (doc) return doc;
    const audio = audioResources[0];
    if (audio) return audio;
    return lesson.resources[0] || null;
  }, [lesson.resources, activePrimaryResId, videoResources, pdfResources, audioResources]);
  ...
  const completedTopBarCount = topBarResources.filter((r) => !!resourceProgressMap[r.id]).length;
    ...
    <div className="flex-1 overflow-y-auto px-4 sm:px-6 pt-3 pb-8 bg-[#f8f9ff] dark:bg-[#090d16] transition-colors">
      <div className={`grid grid-cols-1 gap-4 items-start transition-all duration-200 ${
        ...
        <div className={`flex flex-col gap-3.5 min-w-0 ${
          ...
          {/* ── Top Sleek Resource Switcher Bar (Only Video, or Fallback to Document if 0 Videos) ── */}
          {(topBarResources.length > 0 || (lesson.quizzes && lesson.quizzes.length > 0)) && (
            <div className="bento-card p-1.5 sm:p-2 flex items-center gap-2 shadow-xs">
              {/* Top Bar Resource Progress Badge */}
              {topBarResources.length > 0 && (
                <div 
                  ...
                  title={`Đã hoàn thành ${completedTopBarCount} trên ${topBarResources.length} mục`}
                >
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                  <span>Tiến độ: {completedTopBarCount}/{topBarResources.length}</span>
                </div>
              )}

              {/* Left Scroll Navigation Button */}
              {topBarResources.length > 1 && (
                <button
                  type="button"
          ...
          {activeTab === "audio" && (
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between border-b pb-3 border-slate-100 dark:border-slate-800">
                ...
                <span className="text-[11px] text-slate-400 font-medium">Bấm để chọn track phát hoặc tích để đánh dấu hoàn thành</span>
              </div>
              ...
              {audioResources.length === 0 ? (
                ...
                <div className="flex flex-col gap-2.5">
                  {audioResources.map((audio, index) => {
                    ...
                    const isAudioCompleted = !!resourceProgressMap[audio.id];
                    ...
                    return (
                      <div
                        ...
                        className={`p-3.5 rounded-2xl border transition-all cursor-pointer flex items-center justify-between gap-3 ${
                          isCurrent
                            ...
                            : isAudioCompleted
                            ? "bg-emerald-50/60 dark:bg-emerald-950/20 border-emerald-200/80 dark:border-emerald-800/60 text-slate-800 dark:text-slate-200"
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <button
                            type="button"
                            onClick={(e) => handleToggleResourceComplete(audio.id, e)}
                            className={`p-1 rounded-full transition-all active:scale-90 cursor-pointer shrink-0 ${
                              isAudioCompleted
                            ...
                            title={
                              isAudioCompleted
                                ? "Đã nghe xong - Bấm để hủy hoàn thành"
                                : "Chưa nghe - Bấm để đánh dấu hoàn thành"
                            }
                          >
                            {isAudioCompleted ? (
            ...
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between border-b pb-3 border-slate-100 dark:border-slate-800">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-extrabold text-sm text-slate-900 dark:text-white">
                    Tài Liệu & Bài Tập PDF ({pdfResources.length} tài liệu)
                  </h3>
                </div>
                <span className="text-[11px] text-slate-400 font-medium">Bấm để chọn xem tài liệu trên màn hình chính hoặc tích hoàn thành</span>
              </div>

              {pdfResources.length === 0 ? (
                <div className="p-6 text-center text-xs text-slate-400 border border-dashed rounded-2xl">
                  Chưa gắn tài liệu cho bài học này.
                </div>
              ) : (
                <div className="flex flex-col gap-4">
                  {/* Document List */}
                  <div className="flex flex-col gap-2.5">
                    {pdfResources.map((pdf) => {
                      const isDocCompleted = !!resourceProgressMap[pdf.id];
                      const isCurrentOnStage = currentPrimaryResource?.id === pdf.id;

                      const handleSelectDoc = () => {
                        setActivePrimaryResId(pdf.id);
                        setPreviewDocId(pdf.id);
                      ...
                      return (
                        <div
                          ...
                          className={`p-3.5 rounded-2xl border transition-all cursor-pointer flex items-center justify-between gap-3 ${
                            isCurrentOnStage
                              ? "bg-rose-50 dark:bg-rose-950/40 border-rose-300 dark:border-rose-800 text-rose-900 dark:text-rose-200 shadow-sm ring-2 ring-rose-500/30"
                              : isDocCompleted
                              ? "bg-emerald-50/60 dark:bg-emerald-950/20 border-emerald-200/80 dark:border-emerald-800/60 text-slate-800 dark:text-slate-200"
                              : "bg-[#f8f9ff] dark:bg-slate-800/40 border-slate-200/80 dark:border-slate-700/80 hover:border-rose-300 text-slate-800 dark:text-slate-200"
                          }`}
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            {/* Completion Checkbox */}
                            <button
                              type="button"
                              onClick={(e) => handleToggleResourceComplete(pdf.id, e)}
                              className={`p-1 rounded-full transition-all active:scale-90 cursor-pointer shrink-0 ${
                                isDocCompleted
                                  ? "text-emerald-600 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-900/50"
                                  : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-700"
                            ...
                            <div className="min-w-0">
                              <div className="flex items-center gap-2">
                                <h4 className="font-extrabold text-xs truncate">{pdf.title}</h4>
                                {isDocCompleted && (
                                  <span className="px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 text-[10px] font-bold shrink-0">
                                    Đã xem ✓
                                  </span>
                              ...
                              <span className="text-[10px] text-slate-400">
                                {pdf.resourceType === 2 ? "Tài liệu PDF" : pdf.resourceType === 3 ? "Tài liệu Word" : pdf.resourceType === 4 ? "Hình ảnh" : "Tài liệu đính kèm"}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            {isCurrentOnStage ? (
                              <span className="px-3 py-1 rounded-full bg-rose-500 text-white text-[11px] font-bold shadow-xs">
                                📄 Đang Xem
                              </span>
                            ) : (
                              <button
                                ...
                                className="px-3.5 py-1.5 rounded-full bg-slate-200 dark:bg-slate-700 hover:bg-rose-500 hover:text-white text-slate-700 dark:text-slate-300 text-[11px] font-bold transition-all cursor-pointer flex items-center gap-1.5"
                              >
