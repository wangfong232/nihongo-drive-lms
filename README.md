...
Chạy 1 dòng lệnh sau để khởi tạo container PostgreSQL với volume lưu trữ dữ liệu bền vững:
```bash
...
> 💡 **Lưu ý:** Khi Backend .NET khởi chạy, hệ thống sẽ **tự động chạy migration** (`MigrateAsync()`) và tạo đầy đủ bảng dữ liệu. Bạn cũng có thể cập nhật thủ công bằng lệnh:
```bash
...
#### ⚡ Cách 1: Khởi chạy, Dừng & Cập nhật 1-Click tự động (Windows)
- **Khởi chạy LMS:** Nhấp đúp vào file [`start.bat`](start.bat) hoặc chạy `.\start.bat`.
- **Dừng toàn bộ hệ thống:** Nhấp đúp vào file [`end.bat`](end.bat) để đóng an toàn Backend, Frontend và Docker PostgreSQL.
